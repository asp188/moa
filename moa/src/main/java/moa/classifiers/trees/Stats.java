...
/**
 * Created by qibaoyuan on 15/4/22.
 */
...
/**
 * This class implements some simple statistical functions on arrays of numbers, namely, the mean,
 * variance, standard deviation, covariance, min and max.
 */

public class Stats {

  /**
   * Converts a vector of Numbers into an array of double. This function does not necessarily belong
   * here, but is commonly required in order to apply the statistical functions conveniently, since
   * they only deal with arrays of double. (Note that a Number of the common superclass of all the
   * Object versions of the primitives, such as Integer, Double etc.).
   */
  ...
  public static double[] v2a(Collection v) {
    double[] d = new double[v.size()];
    int i = 0;
    Iterator it = v.iterator();
    while (it.hasNext()) {
      d[i++] = ((Number) it.next()).doubleValue();
    }
//    for (Enumeration e = v.iterator().elements(); e.hasMoreElements(); ) {
//
//    }
    return d;
  }
  ...
  /**
   * Returns the standard error of an array of double, where this is defined as the standard
  ...
  /**
   * Returns a string that compares the root mean square of the data with the standard deviation of
   * the data. This is probably too specialised to be of much general use.
   *
   * @param v contains the data
