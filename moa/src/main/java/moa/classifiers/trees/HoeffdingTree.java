 ...
 *
 * A Hoeffding tree is an incremental, anytime decision tree induction algorithm that is capable of
 * learning from massive data streams, assuming that the distribution generating examples does not
 * change over time. Hoeffding trees exploit the fact that a small sample can often be enough to
 * choose an optimal splitting attribute. This idea is supported mathematically by the Hoeffding
 * bound, which quantiﬁes the number of observations (in our case, examples) needed to estimate some
 * statistics within a prescribed precision (in our case, the goodness of an attribute).</p> <p>A
 * theoretically appealing feature of Hoeffding Trees not shared by other incremental decision tree
 ...
 * inﬁnitely many examples. See for details:</p>
 *
 * <p>G. Hulten, L. Spencer, and P. Domingos. Mining time-changing data streams. In KDD’01, pages
 * 97–106, San Francisco, CA, 2001. ACM Press.</p>
 *
 * <p>Parameters:</p> <ul> <li> -m : Maximum memory consumed by the tree</li> <li> -n : Numeric
 * estimator to use : <ul> <li>Gaussian approximation evaluating 10 splitpoints</li> <li>Gaussian
 * approximation evaluating 100 splitpoints</li> <li>Greenwald-Khanna quantile summary with 10
 * tuples</li> <li>Greenwald-Khanna quantile summary with 100 tuples</li> <li>Greenwald-Khanna
 * quantile summary with 1000 tuples</li> <li>VFML method with 10 bins</li> <li>VFML method with 100
 * bins</li> <li>VFML method with 1000 bins</li> <li>Exhaustive binary tree</li> </ul> </li> <li> -e
 * : How many instances between memory consumption checks</li> <li> -g : The number of instances a
 * leaf should observe between split attempts</li> <li> -s : Split criterion to use. Example :
 * InfoGainSplitCriterion</li> <li> -c : The allowable error in split decision, values closer to 0
 * will take longer to decide</li> <li> -t : Threshold below which a split will be forced to break
 * ties</li> <li> -b : Only allow binary splits</li> <li> -z : Stop growing as soon as memory limit
 * is hit</li> <li> -r : Disable poor attributes</li> <li> -p : Disable pre-pruning</li> <li> -l :
 * Leaf prediction to use: MajorityClass (MC), Naive Bayes (NB) or NaiveBayes adaptive
 * (NBAdaptive).</li> <li> -q : The number of instances a leaf should observe before permitting
 * Naive Bayes</li> </ul>
 *
 * @author Richard Kirkby (rkirkby@cs.waikato.ac.nz)
 * @version $Revision: 7 $
...
public class HoeffdingTree extends AbstractClassifier {
  ...
  public static class Node extends AbstractMOAObject {
    ...
    public void describeSubtree(HoeffdingTree ht, StringBuilder out,
      ...
      this.observedClassDistribution.getSingleLineDescription(out,
                                                              ht.treeRoot.observedClassDistribution
                                                                  .numValues());
  ...
  public static class ActiveLearningNode extends LearningNode {
    ...
    protected AutoExpandVector<AttributeClassObserver>
        attributeObservers =
        new AutoExpandVector<AttributeClassObserver>();
    ...
    public void learnFromInstance(Instance inst, HoeffdingTree ht) {
      if (this.isInitialized == false) {
        this.attributeObservers =
            new AutoExpandVector<AttributeClassObserver>(inst.numAttributes());
      ...
      for (int i = 0; i < inst.numAttributes() - 1; i++) {
        ...
        if (obs == null) {
          obs =
              inst.attribute(instAttIndex).isNominal() ? ht.newNominalClassObserver()
  ...
  public void trainOnInstanceImpl(Instance inst) {
    ...
    if (leafNode instanceof LearningNode) {
      ...
      if (this.growthAllowed
        ...
        if (weightSeen
            - activeLearningNode.getWeightSeenAtLastSplitEvaluation() >= this.gracePeriodOption
                .getValue()) {
  ...
  protected Measurement[] getModelMeasurementsImpl() {
    return new Measurement[]{
        ...
        new Measurement("hbMin", Stats.min(Stats.v2a(this.hbList))),
        new Measurement("hbMax", Stats.max(
            Stats.v2a(this.hbList))), new Measurement("hbMean", Stats.mean(Stats.v2a(this.hbList))),
        new Measurement("hbVar", Stats.variance(
            Stats.v2a(this.hbList)))};
  }
  ...
  protected AttributeClassObserver newNominalClassObserver() {
    AttributeClassObserver
        ...
        (AttributeClassObserver) getPreparedClassOption(this.nominalEstimatorOption);
  ...
  protected AttributeClassObserver newNumericClassObserver() {
    AttributeClassObserver
        ...
        (AttributeClassObserver) getPreparedClassOption(this.numericEstimatorOption);
